...
- **`CpuSimdRuntime` (`TENSOTRON_BACKEND=simd`)** — a hand-written, ILGPU-free managed/SIMD CPU backend (`CpuKernels` = the managed twin of `Kernels`, reusing the same `IOp` structs so XMath transcendentals match the GPU path). The *fast* CPU path for small-net inference/training: batch-1 control-net inference is ~645× faster than ILGPU's **scalar** CPUAccelerator (which carries full per-op dispatch overhead) and beats hand-scalar C# at batch≥8. SIMD is in `CpuKernels.MatMul2D` (`Vector<float>` dot + AXPY paths). **`Auto` (default) falls back to this SIMD backend when no CUDA GPU is present**, not to the ILGPU CPU accelerator. The ILGPU `cpu` accelerator is a slow reference/verification device only (`TENSOTRON_BACKEND=cpu`) — `IlgpuRuntime` prints a loud `WarnIlgpuCpuIsSlow()` banner whenever the live accelerator is `AcceleratorType.CPU`. Regression gate: `tools/run-tests.ps1 -Simd`. See `docs/CPU_SIMD_BACKEND_{PLAN,PROGRESS}.md`.
