...
- The default run uses `Auto`: CUDA if present, else the **managed/SIMD CPU backend** (no longer the slow ILGPU CPU accelerator) — so the full parity suite runs without a GPU. Showcase convergence demos gate on `Cuda.IsAvailable()` via `SkippableFact` and report **Skipped** on a CPU-only box.
...
- **Async runtime, mostly de-overheaded (correctness still first).** Kernels launch on ILGPU's in-order default stream; the runtime `Synchronize()`s **only** at host pulls (`ToArray`/`Item`) plus a safety drain every `FlushEvery` launches (default 64, now a settable knob) — NOT per launch. Already landed: cuBLAS GEMM, a size-bucketed caching allocator (fed by `Dispose`; per-step `DisposeGraph` recycle), shape/stride int buffers cached-by-content and uploaded once (`_intCache`), fused Adam/SGD kernels, chunked parallel reduction for few-output/large-extent gradients (conv bias), device-resident MaxPool argmax, device-resident `ClipGradNorm` (0 syncs). The remaining tiny-model tax is the **per-op host-side graph** — a fresh `Tensor`/`GradNode` per op + a full C# graph rebuild every step (measured **95% host-bound**, ~37 µs/op of pure graph construction). **Escape hatch landed (spike):** `TensorRuntime.Capture(body)` records a fixed-shape step's device launches once and `CapturedGraph.Replay()` re-fires them buffer-to-buffer with no host graph work (software CUDA-Graph) — **~2.5–2.9×** on a small step. Capture taps each `Launch*` (only while capturing; an un-tapped op throws via the `AfterLaunch` `CallerMemberName` assertion), pins trace buffers against pool reuse, and feeds new inputs via `Tensor.Upload`. Spike limits: no buffer reclamation, frozen step-dependent optimizer scalars (Adam bias correction / scheduled LR), no data-dependent index ops (maxpool argmax, gather). Still unoptimized: cross-op kernel fusion. Native CUDA Graph would erase even the per-launch dispatch but ILGPU 1.5.3 exposes no graph-capture API (verified). See `docs/PERFORMANCE_VS_PYTORCH.md`.
